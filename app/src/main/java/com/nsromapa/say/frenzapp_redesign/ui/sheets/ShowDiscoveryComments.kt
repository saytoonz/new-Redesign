...
//        create_comment.isSelected = false
    ...
    private fun setLinearAdapter(commentList: ArrayList<DiscoveryComment>?) {
        ...
        val horizontalAdapter = object : RecyclerView.Adapter<commentsHolder>() {
            ...
            override fun onBindViewHolder(p0: commentsHolder, p1: Int) {
                ...
                p0.comment_time.text = commentList[p1].comment_time
                p0.comment_tv.text = commentList[p1].comment
                p0.likers.text = commentList[p1].comment_likes
                p0.dislikers.text = commentList[p1].comment_dislikes

                p0.like_comment.setOnClickListener{
                    likeComment(commentList[p1].comment_id)
                }

                p0.dislike_comment.setOnClickListener{
                    dislikeComment(commentList[p1].comment_id)
                }


                if (commentList[p1].comment_or_description.equals("comment")
                        && commentList[p1].commenter_id.equals(Utils.getUserUid())){
                ...
                if (commentList[p1].comment_or_description.equals("description")){
